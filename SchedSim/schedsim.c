...
void findWaitingTimeSJF(ProcessType plist[], int n) { 
    int i, min, p_count, current_time, comp_time = 0; 
    int rem_bt[n];
  
    for(i = 0; i < n; i++){
        rem_bt[i] = plist[i].bt;
    }
    while(p_count < n){ // Find process with minimum remaining time at every single time lap.
      for(i = 1; i < n; i++){
      ...
      if (rem_bt[min] == 0){ // Check if its remaining time becomes 0
        p_count = p_count + 1; // Increment the counter of process completion.       
        comp_time = current_time + 1; // Completion time of *current process = current_time +1;      
        plist[min].wt = comp_time - plist[min].bt; // Calculate waiting time for each completed process. wt[i]= Completion time - arrival_time-burst_time
        rem_bt[min] = 9999;
      } 
    current_time = current_time + 1;  // Increment time lap by one.
    } 
} 
